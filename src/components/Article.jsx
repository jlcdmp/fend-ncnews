  ...
  render() {
    ...
    return (
      < article className='article' >
        <Link to='/articles'><button> <span style={{ 'font-size': '28px' }} >🔙</span></button></Link>
        ...
        {voteChange > 10 ? <span style={{ 'font-size': '25px' }} >🔥</span> : null}
        <label>{voteChange}</label>
        <button type='button' onClick={() => this.handleVoteClick(1)}  >
          <span role="img" aria-label="Thumbs-up" style={{ 'font-size': '21px' }} > 👍</span>
        </button>
        <button type='button' onClick={() => this.handleVoteClick(-1)}  >
          <span role="img" aria-label="Thumbs-down" style={{ 'font-size': '21px' }}  > 👎</span>
        </button>
        <button type='button' onClick={() => this.props.save(this.state.article)} >
          <span role="img" aria-label="Star" style={{ 'font-size': '18px' }} >📎</span>
        </button>
